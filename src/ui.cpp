...
// Color wheel pixel buffer — 196×196 RGB565, generated once at boot (~75 KB DRAM).
// 196px diameter exactly meets the arc inner edge (arc inner radius = 98px).
static lv_color_t colorwheel_pixels[196 * 196];

/* --- Main screen icon buffers (36×36 px each) --- */
static lv_color_t white_icon_buf[36 * 36];
static lv_color_t rainbow_icon_buf[36 * 36];
static lv_color_t bookmark_icon_buf[36 * 36];
static lv_color_t star_icon_buf[36 * 36];

/* --- Main screen extra widgets --- */
...
static void generate_white_icon() {
    const int W = 36, H = 36, cx = 17, cy = 17, R2 = 14 * 14;
...
static void generate_rainbow_icon() {
    const int W = 36, H = 36, cx = 17, cy = 17;
    const float R = 14.0f;
    lv_color_t bg = lv_color_hex(0x1E1E1E);
    for (int y = 0; y < H; y++)
        for (int x = 0; x < W; x++) {
...
// Red bookmark: rectangle with a V-notch cut from the bottom.
static void generate_bookmark_icon() {
    const int W = 36, H = 36;
    const int px[] = { 9, 27, 27, 18,  9 };
    const int py[] = { 4,  4, 31, 24, 31 };
    lv_color_t bg = lv_color_hex(0x1E1E1E);
    lv_color_t fg = lv_color_hex(0xCC2222);
    for (int y = 0; y < H; y++)
        for (int x = 0; x < W; x++)
            bookmark_icon_buf[y * W + x] = point_in_poly(px, py, 5, x, y) ? fg : bg;
}

// Yellow 4-point star: outer tips N/E/S/W, inner points at 45° corners.
static void generate_star_icon() {
    const int W = 36, H = 36;
    // 8 vertices alternating outer (radius~15) and inner (radius~7):
    // top, top-right, right, bottom-right, bottom, bottom-left, left, top-left
    const int px[] = { 18, 23, 33, 23, 18, 13,  3, 13 };
    const int py[] = {  3, 13, 18, 23, 33, 23, 18, 13 };
    lv_color_t bg = lv_color_hex(0x1E1E1E);
    lv_color_t fg = lv_color_hex(0xFFCC00);
    for (int y = 0; y < H; y++)
        for (int x = 0; x < W; x++)
            star_icon_buf[y * W + x] = point_in_poly(px, py, 8, x, y) ? fg : bg;
}
...
static void build_main_screen() {
    ...
    lv_obj_set_style_arc_width(brightness_arc, 12, LV_PART_MAIN);
    lv_obj_set_style_arc_color(brightness_arc, lv_color_hex(0xFFEECC), LV_PART_INDICATOR);
    lv_obj_set_style_arc_width(brightness_arc, 12, LV_PART_INDICATOR);
    ...
    /* Brightness % label — top center above grid */
    ...
    lv_obj_align(pct_label, LV_ALIGN_CENTER, 0, -72);

    /* Icon buttons — generate pixel data then place canvases */
    generate_white_icon();
    generate_rainbow_icon();
    generate_bookmark_icon();
    generate_star_icon();

    make_icon_btn(main_screen, white_icon_buf,    -33, -33, cb_white);
    make_icon_btn(main_screen, rainbow_icon_buf,   33, -33, cb_color);
    make_icon_btn(main_screen, bookmark_icon_buf, -33,  33, cb_presets);
    make_icon_btn(main_screen, star_icon_buf,      33,  33, cb_effects);

    /* Power state indicator — small 1/0 at bottom */
    power_label = lv_label_create(main_screen);
    lv_label_set_text(power_label, "1");
    lv_obj_set_style_text_font(power_label, &lv_font_montserrat_14, 0);
    lv_obj_set_style_text_color(power_label, lv_color_hex(0x00CC44), 0);
...
static void build_color_screen() {
    ...
    lv_canvas_set_buffer(color_canvas, colorwheel_pixels, 196, 196, LV_IMG_CF_TRUE_COLOR);
    lv_obj_set_size(color_canvas, 196, 196);
    ...
    // Same geometry as main screen; encoder controls it.
    ...
    lv_obj_set_style_arc_width(color_arc, 12, LV_PART_MAIN);
    lv_obj_set_style_arc_color(color_arc, lv_color_hex(0xFFEECC), LV_PART_INDICATOR);
    lv_obj_set_style_arc_width(color_arc, 12, LV_PART_INDICATOR);
...
static void refresh_preset_labels() {
    ...
    static const uint32_t hex_colors[] = {
        0x444444, 0x888888, 0xFFFFFF, 0x888888, 0x444444,
    };
    for (int i = 0; i < 5; i++) {
        ...
        lv_obj_set_style_text_color(pre_labels[i], lv_color_hex(hex_colors[i]), 0);
    }
}

static void build_presets_screen() {
    presets_screen = make_sub_screen("Presets", cb_back);

    lv_obj_t *hl = lv_obj_create(presets_screen);
    lv_obj_set_size(hl, 200, 22);
    lv_obj_align(hl, LV_ALIGN_TOP_MID, 0, 108);
    lv_obj_set_style_bg_color(hl, lv_color_hex(0x1A3A6A), 0);
    ...
    for (int i = 0; i < 5; i++) {
        ...
        lv_obj_set_style_text_font(pre_labels[i], &lv_font_montserrat_14, 0);
        lv_label_set_long_mode(pre_labels[i], LV_LABEL_LONG_CLIP);
        lv_obj_set_size(pre_labels[i], 190, 20);
...
static void refresh_effects_labels() {
    ...
    static const uint32_t hex_colors[] = {
        0x444444,  // prev-prev — very dim
        0x888888,  // prev       — dim
        0xFFFFFF,  // selected   — bright white
        0x888888,  // next       — dim
        0x444444,  // next-next  — very dim
    };
    for (int i = 0; i < 5; i++) {
        ...
        lv_obj_set_style_text_color(eff_labels[i], lv_color_hex(hex_colors[i]), 0);
    }
}

static void build_effects_screen() {
    effects_screen = make_sub_screen("Effects", cb_back);

    // Blue highlight rect sits behind the selected (center) label.
    // Content: 5 labels at y = 62, 87, 112, 137, 162. Center at y=112.
    lv_obj_t *hl = lv_obj_create(effects_screen);
    lv_obj_set_size(hl, 200, 22);
    lv_obj_align(hl, LV_ALIGN_TOP_MID, 0, 108);
    lv_obj_set_style_bg_color(hl, lv_color_hex(0x1A3A6A), 0);
    lv_obj_set_style_bg_opa(hl, LV_OPA_COVER, 0);
    ...
    // 5 individual labels — created after hl so they render on top of it.
    const int y_base[] = { 62, 87, 112, 137, 162 };
    for (int i = 0; i < 5; i++) {
        eff_labels[i] = lv_label_create(effects_screen);
        lv_obj_set_style_text_font(eff_labels[i], &lv_font_montserrat_14, 0);
        lv_label_set_long_mode(eff_labels[i], LV_LABEL_LONG_CLIP);
        lv_obj_set_size(eff_labels[i], 190, 20);
...
void ui_set_arc_color(uint8_t r, uint8_t g, uint8_t b) {
    ...
    lv_obj_set_style_arc_color(brightness_arc, c, LV_PART_INDICATOR);
    lv_obj_set_style_arc_color(color_arc, c, LV_PART_INDICATOR);
    lv_refr_now(NULL);
}
...
void ui_set_on_state(bool on) {
    lv_label_set_text(power_label, on ? "1" : "0");
    lv_obj_set_style_text_color(power_label,
        on ? lv_color_hex(0x00CC44) : lv_color_hex(0x666666), 0);
}
